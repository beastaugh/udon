...
Udon.curry = function(f) {
    ...
    accumulator = function() {
        ...
        return a2.length >= ar ?
            f.apply(null, a2) :
            Udon.curry.apply(null, [f].concat(a2));
    };
...
Udon.compose = function(fs, ar) {
    var composed = function() {
        var as = Udon._slice.call(arguments, 0),
            i  = fs.length - 1,
            x;
...
Udon.intersperse = function(sep, xs) {
    ...
    while (i--) {
        ys[i] = i % 2 === 0 ? xs[Math.floor(i / 2)] : sep;
    }
...
Udon.drop = function(n, xs) {
    ...
    return (n > len) ? [] : Udon._slice.call(xs, n, len);
};
...
Udon.dropWhile = function(pred, xs) {
    ...
    for (i = 1; i < len; i++) {
        if (pred(xs[i]) === false) return Udon.drop(i, xs);
    }
...
Udon.inits = function(xs) {
    var xcopy  = Udon._slice.call(xs, 0),
        len    = xs.length,
        result = [],
        i;
...
Udon.scanr1 = function(f, x_xs) {
    var len = x_xs.length - 1;
    
    if (Udon.empty(x_xs)) return [];
    
    return Udon.scanr(f, Udon.last(x_xs), Udon._slice.call(x_xs, 0, len));
};
...
Udon.take = function(n, xs) {
    return (n > xs.len) ? xs : Udon._slice.call(xs, 0, n);
};
...
Udon.takeWhile = function(pred, xs) {
    ...
    for (i = 1; i < len; i++) {
        if (pred(xs[i]) === false) return Udon.take(i, xs);
    }
